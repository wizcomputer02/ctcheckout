...
export class TransactionsPage implements OnInit {
  ...
  segmentChanged(event) {
    ...
    if (event.detail.value == 'pending') {
     ...
     this.historyData = this.orderService.orders.filter(res => res['Order']['PaymentStatus'] == "Pending");
     
    }
    if (event.detail.value == 'completed') {
      //this.historyData = this.history.filter(res => res.paymentStatus == "completed");
      this.historyData = this.orderService.orders.filter(res => res['Order']['PaymentStatus'] == "Received");
    }
  }
